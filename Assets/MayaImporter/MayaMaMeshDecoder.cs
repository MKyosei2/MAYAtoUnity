...
{
    /// <summary>
    /// .ma(ASCII)  mesh NodeRecord uMayavŕfR[_B
    ///
    /// ړI:
    /// - Unity Mesh Ɏ܂͈͎͂bV
    /// - Unity ɊTO/vfARawTokens/DecodedExtras Ƃĕێu100%f[^ێvS
    ///
    /// Ή(xXgGtH[g):
    /// - vertices: vt/pt/pnts (float3)
    /// - polyFaces: fc[...]  token stream  f/mu/mn/ed/h \Ȕ͈͂ŉ
    /// - UV: uvst[i].uvsp
    /// - normals: n[...] + (mn/mf) ɂ face-varying normal id ()
    /// - holes: h token ͉͂AUnitýuۑ{ÔݎOpv(oo\) e
    /// </summary>
    public static class MayaMaMeshDecoder
    {
        public sealed class Decoded
        {
            // Expanded stream (face-varying UV/Normals Ή)
            public Vector3[] Vertices;
            public Vector3[] Normals; // null
            public List<Vector2[]> UvSets = new List<Vector2[]>(); // 0..7
            public Color[] Colors; // null

            public int[] Triangles; // single-submesh triangles (submesh partition͏ʂ)
        ...
        public static bool TryDecode(NodeRecord meshShape, MayaImportLog log, out Decoded decoded, out string note)
        {
            ...
            {
                // facesĂ vertices ێP[X̂ŁuvƂĕԂ(|CgNEh)
                decoded = new Decoded
                {
                    Vertices = vertsMaya,
            ...
            for (int fi = 0; fi < faces.Count; fi++)
            {
                ...
                // NOTE: holesꍇłA͊Ô݃t@(holesextrasƂĕێ)
                int n = f.V.Count;
                for (int i = 1; i + 1 < n; i++)
                {
                    int v0 = f.V[0];
        ...
        private static List<string> CleanValueTokens(RawAttributeValue raw)
        {
            ...
            // remove "setAttr -type ..." R quote ł͈킸AlTokeng
