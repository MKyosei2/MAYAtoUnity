...
namespace MayaImporter.Core
{
    ...
    /// Phase C (S):
    /// C-4: ωĂ`lxCNiClipyʉj
    /// C-5: DeterminismiClipnbVjۑčČؖ
    /// C-6: Constraint/IK/Expression ́gUnityɊTOȂĥێi[j
    /// C-7: Audit 1ʂɏWiInspectorEditorXNvgj
    ///
    /// OԍقɋFConstraints/IK/TimeNode͔˂best-effort
    /// O𓊂 import ~߂Ȃi100%ێvzj
    /// </summary>
    public static class MayaPhaseCAutoBakeOnImport
    {
        ...
        private static void ResolveTimeRange_BestEffort(GameObject root, out float start, out float end, out float fps, MayaImportLog log)
        {
            ...
            try
            {
                // TimeNode݊TFstartFrame/endFrame/framesPerSecond  Component
            ...
            out BakeStats stats)
        {
            ...
            for (int i = 0; i < transforms.Length; i++)
            {
                ...
                var st = states[t];
                if (!st.HasAnyChange) continue; // C-4: SωȂTransform̓XLbv
