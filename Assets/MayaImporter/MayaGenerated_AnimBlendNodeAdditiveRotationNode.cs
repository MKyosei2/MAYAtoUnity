...
//  mayaValue  = outEulerMaya
//  unityValue = MayaToUnityConversion.ConvertEulerDegrees(outEulerMaya, options.Conversion)
...
namespace MayaImporter.Generated
{
    ...
    [MayaNodeType("animBlendNodeAdditiveRotation")]
    public sealed class MayaGenerated_AnimBlendNodeAdditiveRotationNode : MayaPhaseCNodeBase
    {
        [Header("Decoded (animBlendNodeAdditiveRotation)")]
        [SerializeField] private bool enabled = true;

        [SerializeField] private Vector3 inputAEulerDeg;
        [SerializeField] private Vector3 inputBEulerDeg;

        [SerializeField] private float weightA = 1f;
        [SerializeField] private float weightB = 1f;
        [SerializeField] private float weight = 1f;

        [SerializeField] private Vector3 outputEulerDegMaya;
        [SerializeField] private Vector3 outputEulerDegUnity;
        ...
        protected override void DecodePhaseC(MayaImportOptions options, MayaImportLog log)
        {
            ...
            inputAEulerDeg = ReadVec3(Vector3.zero,
            ...
            inputBEulerDeg = ReadVec3(Vector3.zero,
            ...
            outputEulerDegMaya = enabled ? (inputAEulerDeg * weightA + inputBEulerDeg * weightB) : inputAEulerDeg;
            outputEulerDegUnity = MayaToUnityConversion.ConvertEulerDegrees(outputEulerDegMaya, options.Conversion);

            // publish
            var outVal = GetComponent<MayaVector3Value>() ?? gameObject.AddComponent<MayaVector3Value>();
            outVal.Set(MayaVector3Value.Kind.Vector, outputEulerDegMaya, outputEulerDegUnity);

            var meta = GetComponent<MayaAnimBlendAdditiveVectorMetadata_Rotation>() ?? gameObject.AddComponent<MayaAnimBlendAdditiveVectorMetadata_Rotation>();
            meta.valid = true;
            meta.enabled = enabled;
            meta.inputA = inputAEulerDeg;
            meta.inputB = inputBEulerDeg;
            meta.weightA = weightA;
            meta.weightB = weightB;
            meta.outputMaya = outputEulerDegMaya;
            meta.outputUnity = outputEulerDegUnity;
            ...
            SetNotes($"{NodeType} '{NodeName}' decoded: enabled={enabled}, inA={inputAEulerDeg}, inB={inputBEulerDeg}, wa={weightA:0.###}, wb={weightB:0.###}, outMaya={outputEulerDegMaya}, outUnity={outputEulerDegUnity}");
            log.Info($"[animBlendNodeAdditiveRotation] '{NodeName}' enabled={enabled} wa={weightA:0.###} wb={weightB:0.###} outMaya=({outputEulerDegMaya.x:0.###},{outputEulerDegMaya.y:0.###},{outputEulerDegMaya.z:0.###})");
        }
    ...
    [DisallowMultipleComponent]
    public sealed class MayaAnimBlendAdditiveVectorMetadata_Rotation : MonoBehaviour
    {
        public bool valid;
        public bool enabled;
        ...
        public Vector3 outputMaya;
        public Vector3 outputUnity;
