...
namespace MayaImporter.Core
{
    ...
    ///
    /// 目的:
    /// - .mb の chunk decode から、mesh(shape)ノードに対して候補payload（float/uint）の参照を付与する
    /// - "名前が出たchunkの周辺（前後ウィンドウ）" を探索して拾い漏れを減らす
    /// - 付与はAdditiveのみ（RawBinaryBytesは常に保持）
    ///
    /// 付与する参照:
    ...
    public static class MayaMbMeshChunkLocator
    {
        ...
        // Depth許容（anchor depth を基準に、どの程度の深さのchunkまで追うか）
        private const int DepthDownTolerance = 2;  // anchorDepth - 2 まで許容
        private const int DepthUpTolerance = 8;    // anchorDepth + 8 まで許容
        ...
        public static void Apply(MayaSceneData scene, MayaImportLog log)
        {
            ...
            // ---------------------------------------------------------
            // 1) mesh候補ノードを集める（NodeType=mesh + Shape名）
            // ---------------------------------------------------------
            ...
            foreach (var kv in scene.Nodes)
            {
                ...
                // mesh扱いのshape名を拾う（best-effort）
                if (!string.IsNullOrEmpty(leaf) && leaf.EndsWith("Shape", StringComparison.Ordinal))
                {
                    meshKeys.Add(rec.Name);
                    AddLeafMap(leafToKeys, leaf, rec.Name);

                    // unknown/transform/empty を mesh に寄せる（後段のbuilderが扱いやすい）
            ...
            // ---------------------------------------------------------
            // 2) chunkを1回走査して anchor を集める
            //    - decodedStrings に DAGパス or leaf が出たら anchor として記録
            // ---------------------------------------------------------
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                for (int r = 0; r < referenced.Count; r++)
                {
                    ...
                    else
                    {
                        ...
                        // BestIndex は「hitが増えたchunk」を採用（後方に寄るほど payload に近いケースが多い）
                        // depthは最後に採用したもの
                        a.BestIndex = i;
            ...
            // anchorが見つからない mesh は、旧ロジック相当で “Shape名だけ” からも拾う（最後の保険）
            if (anchors.Count == 0)
            {
                // fallback: 末尾"Shape"の leaf をキーに、ExtractedStrings から探す
            ...
            // ---------------------------------------------------------
            // 3) anchor毎に window探索して候補chunk refを付与
            // ---------------------------------------------------------
        ...
        private static float ScoreUIntCandidate(MayaBinaryIndex.ChunkInfo ch, int anchorIndex, int idx, int anchorDepth)
        {
            ...
            // Sample heuristic:
            // - polyFacesのように負数が混ざる場合（intとして読むと負になる）→候補として少し加点
            float sampleScore = 0f;
            var u = ch.DecodedUInts;
            if (u != null && u.Length > 0)
            {
                // u は uint なので負数判定はできないが、0/1偏りや巨大値偏りを少し避ける
                int small = 0;
                int huge = 0;
            ...
            Dictionary<string, List<string>> leafToKeys)
        {
            ...
            // ExtractedStringsには大量の文字列が入っていることがあるが、anchorは "最初に見つかったら" 程度で十分
