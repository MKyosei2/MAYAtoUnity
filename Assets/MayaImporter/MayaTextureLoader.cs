...
namespace MayaImporter.Core
{
    ...
    public static class MayaTextureLoader
    {
        ...
        /// <summary>
        /// Phase B:
        /// - UDIM複数タイルは欠損せず保持（B-5）
        /// - Editor上のTextureImporter設定を usage に応じて補正（B-3）
        /// - 画像読めない/見つからない場合も null を返して import を止めない（100%保持思想）
        /// </summary>
            ...
            out MayaTextureLoadInfo info)
        {
            ...
            if (isUdim)
            {
                ...
                // 表示用は 1001 を優先でロード（Unityの標準ShaderではUDIM空間を正しく表示できないため）
                // ただし “データ欠損ゼロ” は MayaUdimSetMetadata が保証する。
            ...
            out string resolvedAssets)
        {
            ...
            {
                // (B-3) usageに応じて TextureImporter 設定補正（Normal/Roughness/Metallic等）
            ...
            try
            {
                ...
                // Normal/Roughness/Metallic は linear 扱いに寄せる
