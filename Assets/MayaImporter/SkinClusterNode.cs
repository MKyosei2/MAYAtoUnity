...
namespace MayaImporter.Deformers
{
    ...
    ///
    /// 100%方針:
    /// - Unityに落とせる範囲は SkinnedMeshRenderer に適用
    /// - Unity制約(1頂点4ウェイト等)で落ちる情報は MayaSkinClusterComponent に完全保持
    /// - 適用結果をターゲットMesh側へ MayaSkinApplyResult として残す（監査・証拠）
    /// </summary>
    ...
    public sealed class SkinClusterNode : MayaNodeComponentBase
    {
        ...
        private static int EstimateMissingBones(GameObject target, string[] influenceNames)
        {
            ...
            // “root fallback” されている骨を missing とみなす（best-effort）
    ...
    /// <summary>
    /// Unityに落としきれない skinCluster 情報を完全保持するコンポーネント。
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class MayaSkinClusterComponent : MonoBehaviour
    {
        ...
        [Tooltip("Sparse list: one entry per (vertex,influence,weight). Unityの4本制限を超えてもここに保持。")]
